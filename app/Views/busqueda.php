...
// Espera recibir: $productos, $termino, $categorias
?>
...
<div class="container-fluid p-0">
  ...
  <div class="banner" style="background-image: url('assets/img/banners.png'); background-size: cover; background-position: center; height: 310px;">
    <div class="d-flex h-100 align-items-center justify-content-center" style="background-color: rgba(0, 0, 0, 0.4);">
      <h1 class="text-white text-center">Resultados para: <?= htmlspecialchars($termino) ?></h1>
    </div>
  </div>
      ...
      <nav class="col-md-3 col-lg-2 d-md-block bg-light sidebar mt-5">
          ...
          <div class="d-flex flex-column w-100 gap-2">
            ...
            <?php if(isset($categorias) && is_array($categorias)): ?>
              <?php foreach($categorias as $cat): ?>
                <a href="<?= base_url('productos/categoria/'.$cat['id_categoria']) ?>" class="btn btn-outline-dark w-100 text-start py-2">
        ...
        <div class="mt-4">
          ...
          <select id="ordenar" class="form-select" onchange="ordenarProductos(this.value)">
            <option value="destacado">Más relevantes</option>
        ...
        <div class="mt-4">
          ...
          <div class="form-check">
            <input class="form-check-input" type="checkbox" value="" id="filtroStock">
            <label class="form-check-label" for="filtroStock">
              Disponibles
            </label>
          </div>
          <div class="form-check">
            <input class="form-check-input" type="checkbox" value="" id="filtroOfertas">
      ...
      <main class="col-md-9 col-lg-10 mt-4">
        <?php if(empty($productos)): ?>
          <div class="alert alert-info">
            No se encontraron productos para "<?= htmlspecialchars($termino) ?>"
          </div>
        <?php else: ?>
          <div class="mb-4">
            <h2>Resultados de búsqueda: <?= htmlspecialchars($termino) ?></h2>
            <p class="text-muted"><?= count($productos) ?> productos encontrados</p>
          </div>
